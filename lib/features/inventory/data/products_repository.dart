...
class ProductsRepository {
  ...
  Future<ProductModel> updateProduct(ProductModel product) async {
    final row = await _supabase
        ...
        .update({
          ...
          if (product.basePrice >= 0) 'base_price': product.basePrice,
  ...
  ProductModel _fromRow(Map<String, dynamic> row) {
    return ProductModel(
      ...
      // Kept on the legacy model for compatibility. It is not used as the
  ...
  String _categoryValue(ProductCategory value) =>
      value == ProductCategory.largeAnimal ? 'large_animal' : value.name;

  ProductCategory _categoryFromValue(String value) {
    if (value == 'large_animal') return ProductCategory.largeAnimal;
  ...
  String _unitValue(ProductUnit value) => value.name;

  ProductUnit _unitFromValue(String value) => ProductUnit.values.firstWhere(
        (item) => item.name == value,
