...
class _EditInvoiceScreenState extends State<EditInvoiceScreen> {
  ...
  void initState() {
    super.initState();
    _draft = InvoiceDraft();
    _discountController.text = widget.invoice.discountAmount.toStringAsFixed(2);
  ...
  Future<void> _requestSave() async {
    if (_draft.items.isEmpty) {
      showAppError(
        context,
        'لا يمكن حفظ الفاتورة بدون أصناف، أضف منتجاً واحداً على الأقل',
      );
      return;
    }

    if (discountAmount < 0 || discountAmount > subtotal) {
      showAppError(context, 'قيمة الخصم غير صحيحة أو أكبر من الإجمالي');
      return;
    }
