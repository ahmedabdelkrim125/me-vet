...
//             _buildCategoryTable(entry.key, entry.value, catalog, boldFont, regularFont),
...
//             pw.Text(_companyContactNumber, style: _valueStyle(boldFont, _green)),
//           ],
//         ),
//         if (logoBytes != null)
...
//     return pw.TextStyle(font: boldFont, fontSize: 10, color: color);
//   }
...
class VehicleStockReportBuilder {
  ...
  static pw.Widget _buildCategoryTable(
    ...
    return _buildTableWrapper(categoryName, headers, rows, boldFont, regularFont, const {
      0: pw.FlexColumnWidth(1.6),
      1: pw.FlexColumnWidth(1.4),
      2: pw.FlexColumnWidth(1.4),
      3: pw.FlexColumnWidth(3),
  ...
  static pw.Widget _buildTodayCategoryTable(
    ...
    final headers = ['الكمية الحالية', 'وقت الإضافة', 'الكمية المضافة', 'اسم الصنف', 'م'];
    final rows = <List<String>>[];
    for (var i = 0; i < items.length; i++) {
      final item = items[i];
      final timeFormatted =
  ...
  static pw.Widget _buildTableWrapper(
    ...
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(title, style: pw.TextStyle(font: boldFont, fontSize: 14, color: _green)),
        pw.SizedBox(height: 8),
        pw.Table(
          border: pw.TableBorder.all(color: _border, width: 0.6),
