...
class CustomerAccountRemoteDataSource {
  ...
  Future<String?> getRepresentativeName({
    required String repId,
  }) async {
    try {
      return await _fetchRepresentativeName(repId);
    } catch (error, stackTrace) {
      if (!_isAuthenticationFailure(error)) rethrow;

      if (kDebugMode) {
        debugPrint(
          '[RepresentativeName] auth failure; attempting one refresh: $error',
        );
        debugPrint('$stackTrace');
      }
      ...
      return _fetchRepresentativeName(repId);
    }
  }
  ...
  Future<String?> _fetchRepresentativeName(String repId) async {
